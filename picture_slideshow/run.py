...
from picture_slideshow.utils import (
    load_params,
    pcloud,
    locate_image_in_folder,
...
def download_batch_as_zip(remotefiles: List[RemoteFile]) -> None:
    kwargs = {"fileids": ",".join([str(x.fileid) for x in remotefiles])}
...
def get_next_batch():
    ...
    log.warning(
        f"last image in its folder {cur_folder_path} at position {cur_folder_position}"
    )
    images_to_download = []
...
def last_downloaded_image() -> str:
    files = sorted(
        [x for x in os.listdir(params["download_folder"]) if ".jpg" in x or ".png" in x]
    )
    print(files)
    if files:
        return files[-1]
...
def retrieve_remote_folder(folder_path="") -> List[RemoteFile]:
    ...
    try:
        if folder_path:
            ...
            folderid_of_year = [
                afolder["folderid"]
                for afolder in root_subfolders
                if afolder["name"] == year
            ][0]

            # go thru this subfolder again to find month
            year_subfolders = pcloud().listfolder(folderid=folderid_of_year)[
                "metadata"
            ]["contents"]
            folderid_of_month = [
                afolder["folderid"]
                for afolder in year_subfolders
                if afolder["name"] == month
            ][0]
            files = pcloud().listfolder(folderid=folderid_of_month)["metadata"][
                "contents"
            ]
    ...
    remote_files = [
        RemoteFile(x["fileid"], x["name"]) for x in files if "image" in x["contenttype"]
    ]
